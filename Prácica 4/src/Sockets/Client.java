...
public class Client {
    public static void main(String[] args) {
        try {
            ...
            String host = consoleInput.nextLine();                                                                      // Leemos la IP
            System.out.println("Enter the port: ");
            int port = consoleInput.nextInt();                                                                          // Leemos el puerto
            Socket sock = new Socket(host, port);                                                                       // Creamos el cliente
            System.out.println("Connected to " + host + " at port " + port);                                            // Imprimimos por pantalla la conexion y el puerto
            PrintWriter pw = new PrintWriter(sock.getOutputStream(), true);                                   // Creamos el printWriter para enviar
            BufferedReader br = new BufferedReader(new InputStreamReader(sock.getInputStream()));                       // Creamos el bufferReader para leer (recibir)
            System.out.println(br.readLine());                                                                          // Leemos la primera lia que recibimos (bienvenida)
